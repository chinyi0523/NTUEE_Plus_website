...
export function handleSubmit(event) {
	...
	if (false) {
		...
		if (r) {
			...
			map.forEach((elements) => {
				if (
					this.state.hasChanged[elements[0]] &&
					this.state[elements[0]] !== undefined
				) {
				...
				if (this.state.hasChanged[`work_${workL}`] === true) {
					...
					;['O', 'P', 'C'].forEach((word) => {
						if (this.state.hasChanged[`work_${word}_${workL}`]) {
							toModify[`work_${word}_${workL}`] = this.state[
								`work_${word}_${workL}`
							]
						}
					})
			...
			if (Object.entries(toModify).length !== 0)
				sta.append('Occupation.Modify', JSON.stringify(toModify))
			if (Object.entries(toRemove).length !== 0)
				sta.append('Occupation.Remove', JSON.stringify(toRemove))
			for (
				var workL = this.state.InitWorkNum + 1;
