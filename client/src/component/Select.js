...
class Select extends Component {
	...
	handleChangeCurrentOption(e) {
		...
		if (this.state.datafunc) {
			this.state.datafunc(this.state.currentOption)
		}
	}
	componentDidMount() {
		this.renderOptions()
		...
		return (
			<div height='100px'>
				<li
					...
					className={
						'Select_main_li d-none d-lg-block ' +
				...
				>
					...
					<ul
						id={this.state.id + '_options_pc'}
						className='Select_ul col-12'
					></ul>
				</li>
				...
				<div className='dropdown d-block d-lg-none '>
					<a
						id={this.state.id + '_select'}
						className='btn dropdown-toggle mt-n1'
						href='#'
						role='button'
					...
					<ul
						id={this.state.id + '_options_mobile'}
						className='dropdown-menu col-12'
						aria-labelledby={this.state.id + '_select'}
